function [Rej_out, A_out, pHperm_out] = run_model_with(p1a,p1b,p1c,p1d,f, pH_space)
...
viscosity=1.23e-6*exp(0.124*TAN*0.001+6.59); %#ok<NASGU> % kept for completeness
k=Sh*D/0.004;
...
for j=1:npH
    ...
    % --- Concentration polarization loop (your original) ---
    Cw = zeros(4,200);
    ...
    try
        for i=1:size(Cw,2)
            syms d0
            eq=Cw(1,i)*p1a*p2a*exp(-d0) + Cw(2,i)*p1c*p2c*exp(-d0) - Cw(3,i)*p1b*p2b*exp(d0) + Xchg;
            phiL=vpasolve(eq,d0);
            ...
            cp_cl=CclR /(p1b*p2b*exp( d1)); %#ok<NASGU>

            C_W_na=(Cna-cp_na)*exp(vf/k)+cp_na;
            C_W_am=(Cam-cp_am)*exp(vf/k)+cp_am;
            C_W_cl=C_W_am+C_W_na; %#ok<NASGU>
        ...
        for iter=1:size(Cw,2)
            ...
            syms fNH3
            eq6=-Damm*Kdamm*e*(fNH3-CammL)/Lm + Kcamm*0.5*(fNH3+CammL)*vf - vf*fNH3/(p1d*p2d);
        ...
        Rej_out(j)    = 1 - TAN_perm/10.7;
        pHperm_out(j) = 14 - (-log10(10^-pOH + x_solve));

        % Pressure/perm. A (kept as in your code, with f injected)
        ...
        ph = -p + (C_W_am + (C_W_am+C_W_na) + C_W_na + C_W_amm - cp_am - cp_na - cp_amm - 0); % osmotic (kept as structure)
        Ph = ph * (8.3144*298)/1000*0.01; % bar
        A_out(j) = lmh/(Ph - (8.3144*298)*( (C_W_am+(C_W_am+C_W_na)+C_W_na+C_W_amm) - (cp_am+cp_na+cp_amm) )/1000*0.01);

    catch
        % In case vpasolve fails: return large penalty downstream
        Rej_out(j)    = NaN;
        A_out(j)      = NaN;
        pHperm_out(j) = NaN;
    end
...
% If any NaN, replace with huge numbers so objective penalizes them
if any(~isfinite(Rej_out)),    Rej_out(:)=nan2pen(Rej_out,1e6); end
if any(~isfinite(A_out)),      A_out(:)=nan2pen(A_out,  1e6); end
if any(~isfinite(pHperm_out)), pHperm_out(:)=nan2pen(pHperm_out,1e6); end
end

function v = nan2pen(v,pen); v(~isfinite(v)) = pen; end
